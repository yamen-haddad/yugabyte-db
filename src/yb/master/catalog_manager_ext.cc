...
Result<TableId> CatalogManager::GetRestoreTargetParentTableForLegacyColocatedDb(
  ...
  // Since parent table name at backup and restore side wil not be the same, we have to generate the
  // parent table name ourselves. Since the backu source database is a legacy 'colocated' database,
  // we have two cases depending on the type of the database at the restore target side:
  // Case 1: Legacy colocated database parent table id =
  // <namespace_id>.colocated.parent.uuid
  // Case 2: Migration to colocation database parent table id =
...
Result<TableId> CatalogManager::GetRestoreTargetTablegroupId(
  ...
  if (IsColocatedDbTablegroupParentTableId(backup_source_tablegroup_id)) {
    ...
    target_tablegroup_id =
        GetColocationParentTableId(GetPgsqlTablegroupId(target_database_oid, tablegroup_oid));
  } else {
    target_tablegroup_id =
        GetTablegroupParentTableId(GetPgsqlTablegroupId(target_database_oid, tablegroup_oid));
  }
