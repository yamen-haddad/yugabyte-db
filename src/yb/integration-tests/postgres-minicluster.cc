...
Status PostgresMiniCluster::InitPostgres() {
  auto pg_ts = RandomElement(this->mini_cluster_->mini_tablet_servers());
  auto port = this->mini_cluster_->AllocateFreePort();
  ...
  ANNOTATE_UNPROTECTED_WRITE(FLAGS_pgsql_proxy_webserver_port) =
      this->mini_cluster_->AllocateFreePort();
  ...
  RETURN_NOT_OK(this->pg_supervisor_->Start());

  this->pg_host_port_ = HostPort(pg_process_conf.listen_addresses, pg_process_conf.pg_port);
  return Status::OK();
}
